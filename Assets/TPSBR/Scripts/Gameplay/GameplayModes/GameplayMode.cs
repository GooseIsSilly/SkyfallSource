...
namespace TPSBR
{
    ...
    public abstract class GameplayMode : ContextBehaviour
    {
        ...
        public void AgentDeath(Agent victim, HitData hitData)
        {
            ...
            // Skip win condition during host migration — the departing host's player being
            // removed temporarily reduces alive count and would falsely end the match.
            if (Context.NetworkGame.IsMigrating == false)
            {
                CheckWinCondition();
            }
        }
        ...
        {
            // Collect all alive (non-eliminated) players into a sorted list first
            // to avoid an unbounded loop when all remaining players are eliminated.
            var alivePlayers = ListPool.Get<PlayerRef>(16);

            foreach (var player in Context.NetworkGame.ActivePlayers)
            {
                if (player == null)
                    continue;
                if (player.Statistics.IsEliminated == true)
                    continue;

                alivePlayers.Add(player.Object.InputAuthority);
            }

            if (alivePlayers.Count == 0)
            {
                // No alive players to spectate — keep current target unchanged.
                ListPool.Return(alivePlayers);
                return;
            }

            var observedPlayerRef = Context.ObservedPlayerRef;
            int currentIndex = alivePlayers.IndexOf(observedPlayerRef);

            int newIndex;
            if (currentIndex < 0)
            {
                // Current target is not in the alive list; jump to first/last.
                newIndex = next ? 0 : alivePlayers.Count - 1;
            }
            else
            {
                newIndex = next
                    ? (currentIndex + 1) % alivePlayers.Count
                    : (currentIndex - 1 + alivePlayers.Count) % alivePlayers.Count;
            }

            observedPlayerRef = alivePlayers[newIndex];
            ListPool.Return(alivePlayers);
