...
namespace TPSBR
{
    public class LiveEventManager : NetworkBehaviour
    {
        ...
        /// <summary>
        /// Static event raised on all clients when any live event fires.
        /// Subscribe to this instead of the instance event when you don't have
        /// a direct reference to LiveEventManager (e.g. SatelliteFireExtinguisher).
        /// </summary>
        public static event Action<LiveEventData> OnAnyEventTriggered;
        ...
        private void RPC_TriggerEventEffects()
        {
            ...
            OnAnyEventTriggered?.Invoke(_activeEvent);
        ...
        /// <summary>
        /// Called by LiveEventSyncService when the backend reports a new scheduled time.
        /// Re-evaluates all events and reschedules the closest upcoming one.
        /// Only executes on the State Authority (Host/Server).
        /// </summary>
        public void RescheduleFromData(LiveEventData updatedData)
        {
            if (!HasStateAuthority) return;
            
            Debug.Log($"[LiveEventManager] Rescheduling due to backend time update: '{updatedData.EventStartTimeEST}'");
            EventHasTriggered = false;
            FindNextUpcomingEvent();
        }
        
        /// <summary>
        /// Immediately fires the active event, regardless of the scheduled time.
        /// Triggered by an admin force-trigger command from the backend.
        /// Only executes on the State Authority (Host/Server).
        /// </summary>
        public void ForceTriggerEvent()
        {
            ...
            if (!IsEventActive)
            {
                Debug.LogWarning("[LiveEventManager] ForceTriggerEvent called but no active event is set.");
                return;
            }
            
            Debug.Log($"[LiveEventManager] Admin force-triggered event: '{_activeEvent.EventName}'");
            TriggerEvent();
        }
