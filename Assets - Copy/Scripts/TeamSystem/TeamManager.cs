...
namespace TPSBR
{
    public class TeamManager : NetworkBehaviour
    {
        ...
        private Dictionary<string, PlayerRef> _userIDToPlayerRef = new Dictionary<string, PlayerRef>();
        ...
        public TeamData CreateOrJoinTeam(string userID, string nickname, string teamID = null, PlayerRef playerRef = default)
        {
            if (!HasStateAuthority)
            {
            ...
            {
                _userIDToPlayerRef[userID] = playerRef;
            }
        ...
        private void SyncTeamToNetwork(TeamData team)
        {
            ...
            for (int i = 0; i < TeamData.MAX_TEAM_SIZE; i++)
            {
                ...
                {
                    string memberUserID = team.MemberUserIDs[i];
                    _userIDToPlayerRef.TryGetValue(memberUserID, out PlayerRef playerRef);

                    var member = new TeamMember
                    {
                        TeamID        = team.TeamID,
                        PlayerRef     = playerRef,
                        IsPartyLeader = memberUserID == team.PartyLeaderUserID,
                        IsAlive       = true,
                    };
