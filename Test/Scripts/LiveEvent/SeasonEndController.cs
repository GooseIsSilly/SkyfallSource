...
namespace TPSBR
{
    public class SeasonEndController : NetworkBehaviour
    {
        ...
        // Tracked so we can cancel it mid-flight when a reset arrives
        private Coroutine _activeSeasonEndCoroutine;
        ...
        private void Start()
        {
            ...
            if (IsInDowntime)
            {
                ...
                _activeSeasonEndCoroutine = StartCoroutine(ShowDowntimeCountdown());
            }
        ...
        public void TriggerSeasonEnd()
        {
            ...
            else if (Object == null || !Object.IsValid)
            {
                ...
                _activeSeasonEndCoroutine = StartCoroutine(SeasonEndSequence());
            }
        }
            ...
            if (_isSeasonEnded)
            {
                ...
                if (secondsUntilSeason > 0)
                {
                    ...
                    else
                    {
                        ...
                        _activeSeasonEndCoroutine = StartCoroutine(ShowImmediateDowntime());
                    }
                }
                else
                {
        ...
        public void ResetDowntimeState()
        {
            ...
            if (Object != null && Object.IsValid && HasStateAuthority)
            {
                NetworkSeasonEnded = false;
                NetworkCountdownShown = false;
                Debug.Log("[SeasonEndController] Broadcasting season end reset to all clients via RPC...");
                RPC_ResetSeasonEndEffects();
            }
            else
            {
                // Standalone / no network authority — apply locally only
                ApplyResetLocally();
            }
        ...
        [Rpc(RpcSources.StateAuthority, RpcTargets.All)]
        private void RPC_ResetSeasonEndEffects()
        {
            Debug.Log($"[SeasonEndController] RPC_ResetSeasonEndEffects received! IsServer: {HasStateAuthority}");
        ...
        private void ApplyResetLocally()
        {
            if (_activeSeasonEndCoroutine != null)
            {
                StopCoroutine(_activeSeasonEndCoroutine);
                _activeSeasonEndCoroutine = null;
                Debug.Log("[SeasonEndController] Cancelled in-flight season end coroutine.");
            }
