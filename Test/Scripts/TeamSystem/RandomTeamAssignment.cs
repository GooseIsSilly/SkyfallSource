...
namespace TPSBR
{
    public class RandomTeamAssignment : NetworkBehaviour
    {
        ...
        private void AssignPlayerToRandomTeam(Player player)
        {
            ...
            {
                TeamManager.Instance.CreateOrJoinTeam(player.UserID, player.Nickname, null, playerRef);
                return;
            }
            ...
            {
                TeamManager.Instance.CreateOrJoinTeam(player.UserID, player.Nickname, availableTeam.TeamID.ToString(), playerRef);
                Debug.Log($"Assigned player {player.Nickname} to existing team {availableTeam.TeamID}");
            }
            else
            {
                var newTeam = TeamManager.Instance.CreateOrJoinTeam(player.UserID, player.Nickname, null, playerRef);
