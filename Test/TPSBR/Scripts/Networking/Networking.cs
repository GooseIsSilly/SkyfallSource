	...
	public class Networking : MonoBehaviour
	{
		...
		private IEnumerator ConnectPeerCoroutine(GamePeer peer, float connectionTimeout = 10f, float loadTimeout = 45f)
		{
			...
			if (peer.Request.Region.HasValue() == true)
			{
				...
				appSettings.FixedRegion = peer.Request.Region;
			...
			if (result.Ok == false)
			{
				...
				else if (result.ShutdownReason == ShutdownReason.Error || result.ShutdownReason == ShutdownReason.Ok)
				{
					// ShutdownReason.Error is too generic — surface the full result for diagnostics.
					ErrorStatus = result.ErrorMessage.HasValue() ? result.ErrorMessage : StringToLabel(result.ShutdownReason.ToString());
				}
				else
				{
		...
		private IEnumerator DisconnectPeerCoroutine(GamePeer peer)
		{
			...
			if (peer.Runner != null && peer.Runner.gameObject != null && peer.Runner.IsShutdown == false)
			{
			...
			if (gameplayScene.IsValid() == true && gameplayScene.isLoaded == true)
			{
		...
		private IEnumerator ShowLoadingSceneCoroutine(bool show, float additionalTime = 1f)
		{
			...
			if (show == true && additionalTime > 0f)
			{
				...
				float waitTime = additionalTime;
				while (waitTime > 0f)
				{
					if (show == true && _currentSession != null && _currentSession.ConnectionRequested == false)
						yield break;
		...
		private sealed class GamePeer
		{
			...
			public bool IsConnected
			{
				get
				{
					...
					// IsCloudReady is false for direct/LAN (non-relay) connections, so only require it
					// when the runner is actually using Photon cloud relay.
					bool requiresCloud = Runner.IsCloudReady == false && Runner.CurrentConnectionType == Fusion.ConnectionType.Relayed;
					if (requiresCloud)
						return false;
