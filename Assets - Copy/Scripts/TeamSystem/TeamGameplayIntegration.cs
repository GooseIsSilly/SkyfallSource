...
namespace TPSBR
{
    public class TeamGameplayIntegration : ContextBehaviour
    {
        ...
        private void UpdateTeamMemberStatus(PlayerRef playerRef, bool isAlive)
        {
            ...
            {
                if (member.PlayerRef == playerRef)
                {
                    // Reflect the alive status back into the networked member via a fresh sync.
                    // The TeamManager will update IsAlive when SyncTeamToNetwork is called after each death.
                    Debug.Log($"[TeamGameplayIntegration] Player {player.Nickname} (Team {teamID}) IsAlive={isAlive}");
                }
            }
        }
