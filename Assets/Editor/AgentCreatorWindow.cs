    ...
    public class AgentCreatorWindow : EditorWindow
    {
        ...
        private void CreateAgent()
        {
            ...
            try
            {
                ...
                // FIX: pass the original asset's path (modelPath) instead of resolving it from the
                // preview-scene instance. AssetDatabase.GetAssetPath returns an empty string for scene
                // instances (they aren't project assets themselves), which silently broke the
                // own-avatar fallback whenever the shared avatar lookup failed.
                SetupAnimator(modelInstance, modelPath);

                Transform modelRoot = FindBone(modelInstance.transform, "root") ?? modelInstance.transform;
                var boneMap = BuildBoneMap(modelRoot);

                // FIX: each wiring step now fails independently instead of throwing and aborting the
                // whole prefab creation (which previously left you with nothing and a stack trace).
                // A failure in one step is logged and skipped so the rest still gets wired.
                TryWire("Character", () => WireCharacter(agentRoot, boneMap));
                TryWire("AnimationController", () => WireAnimationController(agentRoot, modelInstance));
                TryWire("Weapons", () => WireWeapons(agentRoot, boneMap));
                TryWire("Hitboxes", () => WireHitboxes(agentRoot, boneMap));
        ...
        private void TryWire(string stepName, Action wireAction)
        {
            try
            {
                wireAction();
            ...
            {
                Debug.LogError($"[AgentCreator] Failed wiring '{stepName}' - the rest of the prefab was still created, but you'll need to assign this section manually. Error: {e}");
            }
        }
        ...
        private void SetupAnimator(GameObject modelInstance, string modelAssetPath)
        {
            ...
            if (avatar == null)
            {
                avatar = AssetDatabase.LoadAllAssetsAtPath(modelAssetPath).OfType<Avatar>().FirstOrDefault();
                if (avatar != null)
                {
            ...
            if (avatar == null)
            {
                Debug.LogError("[AgentCreator] No Avatar could be assigned (shared avatar missing at "
                    + $"'{SharedAvatarModelPath}' and the model at '{modelAssetPath}' has no Avatar of its own). "
                    + "The agent will not animate until an Avatar is assigned manually.");
            }
        ...
        private void WireCharacter(GameObject agentRoot, Dictionary<string, Transform> boneMap)
        {
            ...
            if (view == null)
            {
                Debug.LogWarning("[AgentCreator] Character component has no '_thirdPersonView' field - skipping third-person view bone wiring. Assign RootBone/HeadTransform/LeftFoot/RightFoot manually.");
                return;
            }
        ...
        private void WireWeapons(GameObject agentRoot, Dictionary<string, Transform> boneMap)
        {
            ...
            if (slots == null)
            {
                Debug.LogWarning("[AgentCreator] Weapons component has no '_slots' field - skipping weapon slot wiring. Assign weapon handles manually.");
                return;
            }
        ...
        private void WireHitboxes(GameObject agentRoot, Dictionary<string, Transform> boneMap)
        {
            ...
            foreach (var kvp in HitboxDefaults)
            {
                ...
                var damageMultiplierProp = bpSo.FindProperty("_damageMultiplier");
                if (damageMultiplierProp != null)
                {
                    damageMultiplierProp.floatValue = kvp.Value.DamageMultiplier;
                }
                else
                {
                    Debug.LogWarning($"[AgentCreator] BodyPart on '{bone.name}' has no '_damageMultiplier' field. Skipping.");
                }

                var isCriticalProp = bpSo.FindProperty("_isCritical");
                if (isCriticalProp != null)
                {
                    isCriticalProp.boolValue = kvp.Value.IsCritical;
                }
                else
                {
                    Debug.LogWarning($"[AgentCreator] BodyPart on '{bone.name}' has no '_isCritical' field. Skipping.");
                }
            ...
            if (hitboxesProp == null)
            {
                Debug.LogWarning("[AgentCreator] HitboxRoot has no 'Hitboxes' field - skipping hitbox list assignment. Assign the created BodyPart hitboxes manually.");
                return;
            }
