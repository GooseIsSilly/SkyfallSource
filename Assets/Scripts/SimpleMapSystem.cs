...
namespace TPSBR
{
    public class SimpleMapSystem : MonoBehaviour
    {
        ...
        [Tooltip("Trailing line that shows where the plane came from. Optional.")]
        [SerializeField] private RectTransform _planeTrail;
        ...
        private void Start()
        {
            ...
            if (_planeMarker != null)
            {
                _planeMarker.gameObject.SetActive(false);
            }

            if (_planeTrail != null)
            {
                _planeTrail.gameObject.SetActive(false);
            }
        }
        ...
        private void FindShrinkingArea()
        {
            ...
            _battleRoyale = FindFirstObjectByType<BattleRoyaleGameplayMode>();
        }
        ...
        /// <summary>
        /// Shows the plane icon on the full-screen map with the correct position and
        /// a rotation that matches the plane's current heading, matching Fortnite's style.
        /// </summary>
        private void UpdatePlaneMarker()
        {
            if (_planeMarker == null)
                return;

            // Lazily resolve BattleRoyaleGameplayMode the first time we need it.
            if (_battleRoyale == null)
            {
                _battleRoyale = FindFirstObjectByType<BattleRoyaleGameplayMode>();
            }

            bool planeActive = _battleRoyale != null && _battleRoyale.Airplane != null;
            _planeMarker.gameObject.SetActive(planeActive);

            if (_planeTrail != null)
            {
                _planeTrail.gameObject.SetActive(planeActive);
            }

            if (!planeActive)
                return;

            Transform planeTf = _battleRoyale.Airplane.transform;

            // ---- Position ----
            Vector2 planeMapPos = WorldToMapPosition(planeTf.position);
            _planeMarker.anchoredPosition = planeMapPos;

            // ---- Rotation (heading on the XZ plane → map Z rotation) ----
            // Negate the Y euler because the map's 2-D axes are flipped relative to world Y rotation.
            float heading = -planeTf.eulerAngles.y;
            _planeMarker.localRotation = Quaternion.Euler(0f, 0f, heading);

            // ---- Optional trailing line showing the flight path ----
            if (_planeTrail != null)
            {
                // Place the trail behind the plane icon: start at planeMapPos and extend
                // in the opposite direction of travel.
                Vector2 backward = (Vector2)(Quaternion.Euler(0f, 0f, heading) * Vector2.down);
                const float TrailLength = 60f;

                _planeTrail.anchoredPosition = planeMapPos + backward * (TrailLength * 0.5f);
                _planeTrail.sizeDelta = new Vector2(_planeTrail.sizeDelta.x, TrailLength);
                _planeTrail.localRotation = Quaternion.Euler(0f, 0f, heading);
            }
        }
