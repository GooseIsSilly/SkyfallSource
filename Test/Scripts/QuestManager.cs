...
namespace TPSBR
{
    public class QuestManager : MonoBehaviour
    {
        ...
        private void CheckAndResetQuests()
        {
            ...
            // Always ensure daily quests are assigned
            bool hasDailyQuests = _playerData.ActiveQuests.Any(q =>
            {
                var def = GetQuestDefinition(q.QuestID);
                return def != null && def.Type == QuestType.Daily;
            });
            ...
            // Always ensure weekly/progression/special quests are assigned
            bool hasWeeklyQuests = _playerData.ActiveQuests.Any(q =>
            {
                var def = GetQuestDefinition(q.QuestID);
                return def != null && (def.Type == QuestType.Weekly || def.Type == QuestType.Progression || def.Type == QuestType.Special);
            });
        ...
        {
            var weeklyQuests = _allQuests.Where(q =>
                q.Type == QuestType.Weekly ||
                q.Type == QuestType.Progression ||
                q.Type == QuestType.Special).ToList();
        ...
        public void UpdateQuestProgress(QuestRequirementType requirementType, int amount, object additionalData = null)
        {
            ...
            foreach (var questProgress in activeQuests)
            {
                ...
                else if (requirementType == QuestRequirementType.SurviveTime || requirementType == QuestRequirementType.TravelDistance)
                {
                    // These are reported as cumulative totals — take the maximum to avoid double-counting.
                    questProgress.CurrentProgress = Mathf.Max(questProgress.CurrentProgress, amount);
                }
                else
                {
