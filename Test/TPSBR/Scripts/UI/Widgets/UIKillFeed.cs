...
namespace TPSBR.UI
{
	...
	public class UIKillFeed : UIWidget
	{
		...
		public void HideAll()
		{
			// Stop the move coroutine before clearing state so _moveRoutine
			// is never left non-null after the lists are wiped, which would
			// permanently block OnTick from processing pending feeds.
		...
		private IEnumerator MoveFeeds_Coroutine()
		{
			...
			// Guard: HideAll() may have cleared the routine reference already.
			if (_moveRoutine != null)
			{
				_moveRoutine = null;
			}
